...
|---------|-----------|-----------|
| `cg_fetch_ohlc` | False | شموع من CoinGecko — **مغلق**: Binance وحده مصدر الشموع |
| `cg_min_interval_sec` | 6.0 | مباعدة بين طلبات CoinGecko (للكون والميتاداتا) |
...
### الشموع من Binance وحده

CoinGecko للكون والميتاداتا (الترتيب، القيمة السوقية، الفوليوم) فقط. مسار الشموع
لا يمر به إطلاقاً.

السبب قياس لا تقدير: الـPublic API موثّق بـ5-15 طلباً/دقيقة «حسب ظروف الاستخدام»
بلا رقم ثابت، والطلبات الفاشلة تُحتسب ضمن الحد. تشغيلان حيّان ردّا 429 بعد 13 ثم
بعد 7 طلبات رغم مضاعفة المباعدة من 2.5 إلى 6 ثوانٍ. لا قيمة لـ`cg_min_interval_sec`
تجعل هذا المسار موثوقاً.

العملة بلا زوج USDT على Binance تُسجَّل `no_ohlcv`: لا شموع، لا تحليل، لا إشارة،
ولا مركز ورقي. وهي **لا تُفشل الدورة** — نقص تغطية معلوم يختلف عن جودة بيانات
مشكوك فيها، والتقرير يفصلهما: `no_ohlcv` مقابل `skipped` (سقوط مصدر أو نفاد ميزانية).

`cg_fetch_ohlc=True` يعيد المسار القديم إن أضفنا مفتاح CoinGecko demo لاحقاً
(ترويسة `x-cg-demo-api-key`، حد ثابت أعلى). غير مفعّل الآن.

### استبعاد العملات المستقرة بالهوية لا بالرمز

`STABLE_COIN_IDS` يُطابق `coin_id` الفريد من CoinGecko، و`STABLE_SYMBOLS` يبقى
للرموز المعروفة. الرموز تتصادم — عملتان بنفس الرمز واردتان — فاستبعاد بالرمز وحده
يُسقط عملة سليمة من الإشارات.

المستبعدة حالياً بالـid، وكلها تأكّدت من الكون الحي: `global-dollar` (USDG)،
`hashnote-usyc` (USYC)، `ondo-us-dollar-yield` (USDY).

وقرب الدولار ليس دليلاً في الاتجاهين: FIL كان بـ$0.975 وهو Filecoin، و`M` بـ$1.50
هو MemeCore. لذلك لا يُخمَّن رمز: العملة المجهولة تبقى داخل الإشارات حتى يُتحقق من هويتها عبر

```powershell
python check_live.py --identify
```

يطبع `coin_id` والاسم والسعر وتغيّر 30 يوماً لكل عملة قرب الدولار في الكون الحي،
بطلب واحد وبلا دورة كاملة.
...
### المركز القائم يسبق الاكتشاف في ميزانية الشموع

`max_ohlcv_per_cycle` أصغر من الكون (40 من 100)، وإدارة المركز الورقي تعتمد كلياً على
شموع هذه الدورة: `update_paper` يقرأ من `ohlcv_cache` وحدها. فعملة لم تُسحب شموعها لا
تتحرك لها MFE/MAE، ولا يُفحص وقفها ولا أهدافها، **ولا تنتهي نافذة دخولها** لأن فحص
الانتهاء داخل معالجة الشمعة نفسها. النتيجة أن الصفقة تتجمّد بصمت وتُقرأ لاحقاً كأنها
معلومة عن السوق.

لذلك `_priority_order()` يضع العملات ذات مركز `OPEN` أو `PENDING` في المقدمة، قبل
الداخل الجديد وصاعدي الترتيب. وفوق ذلك `cycle_coins()` يعامل `max_ohlcv_per_cycle`
سقفاً على **الاكتشاف** لا على الإدارة: تُسحب شموع كل مركز قائم أولاً، ثم يأخذ الاكتشاف
ما تبقّى. فلو صار عدد المراكز أكبر من الميزانية تُدار كلها ويصير الاكتشاف صفراً، لا أن
يُقصّ المركز الزائد. صفقة لا تُدار ليست قياساً ناقصاً بل ليست قياساً أصلاً، واكتشاف
عملة جديدة لا يسبق ذلك. وحين تستوعب المراكز الميزانية كاملةً تسجّل الدورة تحذيراً
صريحاً، لأن الاكتشاف يكون قد توقف. وبما أن الشموع من Binance وحده فالطلبات الزائدة
تتباعد بـ`binance_min_interval_sec` ولا تمسّ ميزانية CoinGecko.
...
أفضل وأسوأ صفقة، متوسط المدة، متوسط MFE/MAE، رأس المال الورقي، أقصى تراجع، والأداء
**مفصّلاً حسب حالة السوق** — وهو ما يكشف إن كان المحرك يربح في BULL ويخسر في BEAR
مثلاً. نفس الأرقام متاحة على `/api/top100` تحت `paper`.

وقبل أن تُغلق أول صفقة لا يكون التقرير فارغاً: يسرد MFE/MAE الجاريين لكل مركز مفتوح،
فأول أيام الاختبار تُقرأ ولا تُنتظر.

### أقصى التراجع و MFE/MAE

`max_drawdown()` يقيس القمة إلى القاع على **منحنى رأس المال المحقق**: يُعاد بناء رأس
المال صفقةً بصفقة بترتيب زمن الإغلاق (`realized_equity_curve()`) لا من جدول
`paper_equity`، لأن ذلك الجدول صف واحد لكل يوم فتذوب فيه الحركة بين صفقتين أُغلقتا في
اليوم نفسه. القيمة الأخيرة على المنحنى تطابق `equity()` لأن حاصل الضرب لا يتأثر
بالترتيب، لكن التراجع يتأثر به فعلاً، ولذلك يُحسب على التسلسل.

الرقم **تحفّظي بطبعه**: `equity()` لا يحتسب المراكز المفتوحة، فمركز هابط لم يُغلق بعد
لا يظهر في أقصى التراجع. حركته تظهر في MAE الخاص به، ولهذا يُقرأ المقياسان معاً — أحدهما
يصف رأس المال والآخر يصف المركز.

و MFE محكوم بسياسة الشمعة الواحدة نفسها: إن ضربت الشمعة الوقف ثم ارتفعت، فالافتراض أن
الوقف نُفِّذ أولاً، والارتفاع بعده حركةٌ لمركز مُغلق فلا يُحتسب MFE. هذا يمنع تجميل
الأرقام من حيث لا يُقصد.
